...
/* LXOP functions: */
/* Checkout pins the pool through final release; shutdown refuses new checkouts. */
extern P_OSAL_OP wmt_lib_get_free_op(VOID);
/* Return an unsubmitted checkout, or recycle after the final submitted reference. */
extern INT32 wmt_lib_put_op_to_free_queue(P_OSAL_OP pOp);
extern PVOID wmt_lib_alloc_op_data(P_OSAL_OP pOp, SIZE_T size);
extern VOID wmt_lib_put_op_ref(P_OSAL_OP pOp);
extern MTK_WCN_BOOL wmt_lib_put_act_op(P_OSAL_OP pOp);
/* Consumes pOp; result holds submitted fields or the completed reply. */
extern MTK_WCN_BOOL wmt_lib_put_act_op_result(P_OSAL_OP pOp, P_OSAL_OP_DAT result);
/* Retains the sender reference, including on failure; release after copying outputs. */
extern MTK_WCN_BOOL wmt_lib_submit_op_result(P_OSAL_OP pOp, P_OSAL_OP_DAT result);
...
extern INT32 wmt_lib_send_cmd(const UINT8 *command);
extern ssize_t wmt_lib_read_cmd(char __user *buffer, size_t count);
extern VOID wmt_lib_cancel_cmd(VOID);
...
extern INT32 wmt_lib_parse_hif(ULONG hifconf, P_WMT_HIF_CONF config);
extern VOID wmt_lib_set_hif(const WMT_HIF_CONF *config);
...
extern INT32 wmt_lib_set_rom_patch_info(struct wmt_rom_patch_info *PatchInfo, ENUM_WMTDRV_TYPE_T type);
/* Callers supply room for the 256-byte name and four-byte address. */
extern INT32 wmt_lib_get_rom_patch_info(SIZE_T type, PUINT8 name, PUINT8 address);
...
/* Only the publishing wmtd core dispatcher may borrow this pointer. */
