...
static UINT32 hif_info;
/* Serialize initialization and publication of a pending display power-on. */
static DEFINE_MUTEX(g_hif_lock);
UINT8 gWmtClose;
...
static INT32 wmt_fb_notifier_callback(struct notifier_block *self, ULONG event, PVOID data)
{
	...
	case FB_BLANK_UNBLANK:
		...
		mutex_lock(&g_hif_lock);
		if (hif_info == 0) {
			atomic_set(&g_late_pwr_on_for_blank, 1);
			mutex_unlock(&g_hif_lock);
			break;
		}
		mutex_unlock(&g_hif_lock);
		schedule_work(&gPwrOnOffWork);
...
static INT32 wmt_dev_parse_dynamic_dump(char *buffer, UINT32 *values, size_t count)
{
	char *field;
	size_t index = 0;
	INT32 ret;

	/* An empty property clears the table; a final separator is optional. */
	if (!*buffer)
		return 0;
	...
	while ((field = strsep(&buffer, "/")) != NULL) {
		if (!*field || index >= count)
			return -EINVAL;
		ret = kstrtou32(field, 0, &values[index]);
		if (ret)
			return ret;
		index++;
		if (buffer && !*buffer)
...
LONG WMT_unlocked_ioctl(struct file *filp, UINT32 cmd, ULONG arg)
{
	...
	case WMT_IOCTL_SET_STP_MODE:	/* stp/hif/fm mode */
		...
		do {
			...
			iRet = wmt_lib_parse_hif(arg, &config);
			if (iRet != 0) {
				WMT_INFO_FUNC("invalid hif configuration (%lu)\n", arg);
				break;
			}
			...
			wmt_lib_set_hif(&config);
			osal_memcpy(&pOp->op.au4OpData[0], &config, sizeof(config));
	...
	case WMT_IOCTL_LPBK_TEST:
		do {
			...
			if (effectiveLen > WMT_LPBK_BUF_LEN) {
				iRet = -EFAULT;
			...
			lpbk_buf = wmt_lib_alloc_op_data(pOp, WMT_LPBK_BUF_LEN);
			if (!lpbk_buf) {
				wmt_lib_put_op_to_free_queue(pOp);
				iRet = -ENOMEM;
				break;
			}
			if (copy_from_user(lpbk_buf, (PVOID)arg + sizeof(effectiveLen), effectiveLen)) {
				wmt_lib_put_op_to_free_queue(pOp);
				iRet = -EFAULT;
				break;
			}
			...
			pOp->op.au4OpData[1] = (SIZE_T)lpbk_buf;
			osal_memcpy(&lpbk_tail, lpbk_buf + (effectiveLen >= 4 ? effectiveLen - 4 : 0),
				    effectiveLen >= 4 ? 4 : effectiveLen);
			...
			if (bRet == MTK_WCN_BOOL_FALSE) {
				WMT_WARN_FUNC("OPID(%d) type(%zu) buf tail(0x%08x) fail\n",
					      result.opId, result.au4OpData[0], lpbk_tail);
				iRet = -1;
				wmt_lib_put_op_ref(pOp);
				break;
			}
			WMT_INFO_FUNC("OPID(%d) length(%zu) ok\n", result.opId, result.au4OpData[0]);
			iRet = result.au4OpData[0];
			if (result.au4OpData[0] > WMT_LPBK_BUF_LEN) {
				iRet = -EFAULT;
				WMT_ERR_FUNC("length is too long\n");
				wmt_lib_put_op_ref(pOp);
				break;
			}
			if (copy_to_user((PVOID)arg + sizeof(effectiveLen) + sizeof(UINT8[2048]), lpbk_buf, iRet)) {
				iRet = -EFAULT;
			}
			wmt_lib_put_op_ref(pOp);
	...
	case WMT_IOCTL_ADIE_LPBK_TEST:
		do {
			...
			if (copy_to_user((PVOID)arg + sizeof(SIZE_T), lpbk_buf, iRet)) {
				iRet = -EFAULT;
			}
			wmt_lib_put_op_ref(pOp);
		} while (0);
	...
	case WMT_IOCTL_SET_PATCH_NUM:
		iRet = wmt_dev_set_patch_num(arg);
		break;
	case WMT_IOCTL_SET_PATCH_INFO:
		do {
			...
			iRet = wmt_dev_set_patch_info(&wMtPatchInfo);
			if (iRet)
				break;

			WMT_DBG_FUNC(
				"patch dl seq %d,name %s,address info 0x%02x,0x%02x,0x%02x,0x%02x\n",
			     wMtPatchInfo.dowloadSeq, wMtPatchInfo.patchName,
	...
	case WMT_IOCTL_SEND_BGW_DS_CMD:
		do {
			...
			desense_buf = wmt_lib_alloc_op_data(pOp, effectiveLen);
			if (!desense_buf) {
				wmt_lib_put_op_to_free_queue(pOp);
				iRet = -ENOMEM;
				break;
			}
			if (copy_from_user(desense_buf, (PVOID)arg, effectiveLen)) {
				wmt_lib_put_op_to_free_queue(pOp);
				iRet = -EFAULT;
				break;
			}
			...
			pOp->op.au4OpData[0] = effectiveLen;	/* packet length */
			pOp->op.au4OpData[1] = (SIZE_T)desense_buf;
			pSignal->timeoutValue = MAX_EACH_WMT_CMD;
			WMT_INFO_FUNC("OPID(%d) start\n", pOp->op.opId);
			if (DISABLE_PSM_MONITOR()) {
				WMT_ERR_FUNC("wake up failed,opid(%d) abort\n", pOp->op.opId);
	...
	case WMT_IOCTL_DYNAMIC_DUMP_CTRL:
		do {
			...
			iRet = wmt_dev_parse_dynamic_dump((char *)pBuf, int_buf,
							 ARRAY_SIZE(int_buf));
			if (iRet)
				WMT_ERR_FUNC("invalid dynamic dump data(%d)\n", iRet);
			else
				iRet = wmt_plat_set_dynamic_dumpmem(int_buf);
			kfree(pBuf);
		} while (0);
		break;
	case WMT_IOCTL_SET_ROM_PATCH_INFO:
		do {
			...
			iRet = wmt_lib_set_rom_patch_info(&wmtRomPatchInfo, wmtRomPatchInfo.type);
			if (iRet)
				break;
...
static INT32 WMT_init(VOID)
{
...
#else
	ret = mtk_wcn_hif_sdio_drv_init();
	if (ret) {
		WMT_ERR_FUNC("hif_sdio_drv_init() fails (%d)\n", ret);
		goto error;
	}
#endif
	ret = stp_drv_init();
	...
	ret = cdev_add(&gWmtCdev, devID, WMT_DEV_NUM);
	if (ret) {
