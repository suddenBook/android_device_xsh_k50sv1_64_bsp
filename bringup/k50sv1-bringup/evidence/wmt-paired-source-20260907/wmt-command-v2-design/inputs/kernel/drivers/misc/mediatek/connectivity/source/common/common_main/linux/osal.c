...
INT32 _osal_fifo_init(OSAL_FIFO *pFifo, PUINT8 buf, UINT32 size)
{
	...
	fifo = kzalloc(sizeof(*fifo), GFP_ATOMIC);
	if (!fifo)
		return -ENOMEM;
	if (!buf) {
...
INT32 osal_fifo_init(P_OSAL_FIFO pFifo, UINT8 *buffer, UINT32 size)
{
	...
	if (pFifo->pFifoBody != NULL) {
		...
		osal_fifo_deinit(pFifo);
	}

	return pFifo->FifoInit(pFifo, buffer, size);
}
...
static VOID osal_op_history_print_work(struct work_struct *work)
{
	...
	/* The next snapshot may reuse the buffer only after its last read. */
...
VOID osal_op_history_print(struct osal_op_history *log_history, PINT8 name)
{
	...
	spin_lock_irqsave(&log_history->lock, flags);
	if (!log_history->queue || log_history->dump_busy)
		goto out;
	...
	dump_queue = dump_ring_buffer->base;
	osal_snprintf(log_history->name, sizeof(log_history->name), "%s", name);
	osal_memcpy(dump_queue, log_history->queue,
		   sizeof(*log_history->queue) * ring_buffer->max_size);
	*dump_ring_buffer = *ring_buffer;
	dump_ring_buffer->base = dump_queue;
	log_history->dump_busy = true;
	/* Enqueue before unlocking so teardown can join every published dump. */
	schedule_work(&log_history->dump_work);
out:
	spin_unlock_irqrestore(&log_history->lock, flags);
}
