	...
	if (WMTDRV_TYPE_WMT > drvType || WMTDRV_TYPE_ANT == drvType) {
		if (gpWmtFuncOps[drvType] && gpWmtFuncOps[drvType]->func_on) {
			...
			if (drvType == WMTDRV_TYPE_WIFI) {
				...
				WRITE_ONCE(pOp->op.opId, WMT_OPID_WLAN_PROBE);
	...
	if (gMtkWmtCtx.eDrvStatus[drvType] != DRV_STS_FUNC_ON) {
		...
		if (gpWmtFuncOps[drvType] && gpWmtFuncOps[drvType]->func_off) {
			...
			if (drvType == WMTDRV_TYPE_WIFI) {
				...
				WRITE_ONCE(pOp->op.opId, WMT_OPID_WLAN_REMOVE);
				if (wmt_lib_put_worker_op(pOp) == MTK_WCN_BOOL_FALSE) {
					WMT_WARN_FUNC("put to activeWorker queue fail\n");
					atomic_set(&g_wifi_on_off_ready, 0);
					return -4;
...
{
	INT32 iret;
	UINT32 value = pWmtOp->au4OpData[2];
	...
	iret = wmt_core_reg_rw_raw(pWmtOp->au4OpData[0],
				   pWmtOp->au4OpData[1],
				   &value, pWmtOp->au4OpData[3]);
	pWmtOp->au4OpData[2] = value;
...
static INT32 opfunc_efuse_rw(P_WMT_OP pWmtOp)
{
	...
	osal_memcpy(&WMT_EFUSE_CMD[8], &pWmtOp->au4OpData[2], 4);	/* value */
...
static INT32 opfunc_gps_mcu_ctrl(P_WMT_OP pWmtOp)
{
	...
	do {
		...
		iRet = wmt_core_rx(p_rx_buf, rx_data_buf_len + osal_sizeof(WMT_GPS_MCU_CTRL_CMD),
				   &u4ReadSize);
		if (iRet || u4ReadSize < osal_sizeof(WMT_GPS_MCU_CTRL_CMD) ||
		    u4ReadSize > rx_data_buf_len + osal_sizeof(WMT_GPS_MCU_CTRL_CMD) ||
		    p_rx_buf[1] != WMT_GPS_MCU_CTRL_CMD[1]) {
			WMT_ERR_FUNC("gps mcu ctrl rx EVT fail(%d),size(%d)\n", iRet, u4ReadSize);
			break;
		}
		rx_data_len = p_rx_buf[2] | (p_rx_buf[3] << 8);
		if (rx_data_len > u4ReadSize - osal_sizeof(WMT_GPS_MCU_CTRL_CMD)) {
			WMT_ERR_FUNC("gps mcu ctrl truncated EVT,size(%d),payload(%d)\n",
				     u4ReadSize, rx_data_len);
			break;
		}
		osal_memcpy(p_rx_data_buf, p_rx_buf + osal_sizeof(WMT_GPS_MCU_CTRL_CMD), rx_data_len);
		pWmtOp->au4OpData[4] = rx_data_len;
