...
static void *set_firmware_log(void *argument)
{
    ...
    /* This kernel returns 1 for a successful disable, and 0 for enable. */
...
static void check_optional_controls(int fd, struct optional_controls *controls)
{
    ...
    if (strcmp(fwlog, controls->fwlog) &&
        (!controls->log.created || atomic_load(&controls->log.finished))) {
        ...
        controls->log.fd = fd;
        controls->log.enabled = !strcmp(fwlog, "yes");
...
static int handle_command(int fd, const char *directory, const struct wmt_command *command)
{
    ...
    struct wmt_firmware_result firmware = {0};
...
int main(int argc, char **argv)
{
    ...
    bool bound = false, powered = false, advertised = false;
