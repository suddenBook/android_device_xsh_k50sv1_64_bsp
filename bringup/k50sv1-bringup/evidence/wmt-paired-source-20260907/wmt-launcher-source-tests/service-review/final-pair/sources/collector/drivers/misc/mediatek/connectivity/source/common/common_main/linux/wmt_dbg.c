...
INT32 wmt_dbg_cmd_test_api(ENUM_WMTDRV_CMD_T cmd)
{
	...
	case WMTDRV_CMD_FWTRACE_TEST:
		...
		break;
	default:
		payload = wmt_lib_alloc_op_data(pOp, sizeof(gCoexBuf.buffer));
		if (!payload) {
		...
		pOp->op.au4OpData[2] = (SIZE_T)payload;
	...
	if ((cmd != WMTDRV_CMD_ASSERT) &&
		...
		mutex_lock(&g_wmt_coex_lock);
		if (bRet == MTK_WCN_BOOL_FALSE || result.au4OpData[3] > sizeof(gCoexBuf.buffer)) {
			gCoexBuf.availSize = 0;
		} else {
			osal_memcpy(gCoexBuf.buffer, payload, result.au4OpData[3]);
			gCoexBuf.availSize = result.au4OpData[3];
			WMT_INFO_FUNC("gCoexBuf.availSize = %d\n", gCoexBuf.availSize);
		}
		mutex_unlock(&g_wmt_coex_lock);
	}
...
/* Caller holds g_dbg_emi_lock. One means disabled; negative values are errors. */
...
static INT32 wmt_internal_loopback(INT32 count, INT32 max)
{
	...
	if (max < 0 || max > sizeof(lpbk_buffer.out_payload) || count < 0)
		return -1;
	...
	for (loop = 0; loop < count; loop++) {
		...
		payload = wmt_lib_alloc_op_data(pOp, sizeof(lpbk_buffer.out_payload));
		if (!payload) {
			wmt_lib_put_op_to_free_queue(pOp);
			ret = -1;
			break;
		}
		osal_memcpy(payload, lpbk_buffer.out_payload, max);
		pSignal = &pOp->signal;
		pOp->op.opId = WMT_OPID_LPBK;
		pOp->op.au4OpData[0] = lpbk_buffer.payload_length;	/* packet length */
		pOp->op.au4OpData[1] = (SIZE_T)payload;
		pSignal->timeoutValue = MAX_EACH_WMT_CMD;
		...
		ret = wmt_lib_submit_op_result(pOp, &result);
		ENABLE_PSM_MONITOR();
		...
		osal_memcpy(lpbk_buffer.in_payload, payload, max);
		wmt_lib_put_op_ref(pOp);

		ret = result.au4OpData[0];
		/*<3> compare result */
...
static INT32 wmt_dbg_set_mcu_clock(INT32 par1, INT32 par2, INT32 par3)
{
	...
	ret = wmt_lib_put_act_op_result(pOp, &result);
	ENABLE_PSM_MONITOR();
	if (ret == MTK_WCN_BOOL_FALSE) {
		WMT_WARN_FUNC("OPID(%d) kind(%zu)fail(%d)\n",
				result.opId, result.au4OpData[0], ret);
		return -3;
	}
	...
	if (*f_pos > 0)
		...
		if (gCoexBuf.availSize <= 0) {
			...
			*f_pos += retval;
			...
			/* Each byte needs five characters; reserve newline and NUL. */
			max_num = min_t(size_t, sizeof(gCoexBuf.buffer),
					(sizeof(msg_info) - 2) / 5);
