...
objects = [path for path in changed if path.endswith('.c') and '/test/' not in path
           and not path.startswith('tools/testing/')]
assert objects and all(path.startswith('drivers/misc/mediatek/connectivity/')
                       or path == 'drivers/base/platform.c' for path in objects)
