"""Freeze the reviewed kernel, source launcher, vendor recipe and evidence for build19."""
...
output = trial / 'build19-input-update.json'
...
status = subprocess.check_output(['systemctl', '--user', 'show', 'k50sv1-build19.service',
                                 '--property=ActiveState', '--value'], text=True).strip()
assert status not in ('active', 'activating')
validation_path = root / 'work/k50sv1-bringup/evidence/wmt-old-clang-20260907/source-validation.json'
...
clean(root / 'work')
previous = json.loads((trial / 'build18-input-update.json').read_text())
...
with (trial / 'build19-prebuild-source-gates.log').open('xb') as log:
...
with (trial / 'build19-prebuild-source-state.txt').open('xb') as state:
    subprocess.run([str(project / 'work/k50sv1-bringup/tools/capture-build-input-state.sh'),
                    '--repo-manifest', str(trial / 'build19-prebuild-repo-manifest.xml')],
                   cwd=project, env=environment, stdout=state, check=True, timeout=300)
print('PASS: build19 reviewed inputs frozen and full source preflight passed', flush=True)
