...
/* Protect active pointers and the terminal result/cancellation decision. */
static DEFINE_SPINLOCK(g_wmt_op_lock);
/* These guards outlive gDevWmt and every operation borrowed from its pool. */
...
/* Completed stages for the outer lifecycle caller's failure unwind. */
...
/* All terminal decisions, including completion reuse, hold g_wmt_cmd_lock. */
...
INT32 wmt_lib_parse_hif(ULONG hifconf, P_WMT_HIF_CONF config)
{
	UINT32 val;
	WMT_HIF_CONF parsed = { 0 };
	P_WMT_HIF_CONF pHif = &parsed;
...
VOID wmt_lib_set_hif(const WMT_HIF_CONF *config)
{
	gDevWmt.rWmtHifConf = *config;
	switch (config->hifType) {
	case WMT_HIF_UART:
...
/* Parse the one private copy; no userspace bytes are fetched during commit. */
...
/* Lock order: command broker, then exactly one metadata cache. */
...
MTK_WCN_BOOL wmt_lib_handle_idc_msg(conn_md_ipc_ilm_t *idc_infor)
{
	...
	if (!idc_infor || !idc_infor->local_para_ptr ||
	    idc_infor->local_para_ptr->msg_len <= osal_sizeof(struct local_para))
		return MTK_WCN_BOOL_FALSE;
	...
	payload = wmt_lib_alloc_op_data(lxop, osal_sizeof(msg_len) + msg_len);
	if (!payload) {
		wmt_lib_put_op_to_free_queue(lxop);
		return MTK_WCN_BOOL_FALSE;
	}
	osal_memcpy(payload, &msg_len, osal_sizeof(msg_len));
	/* Preserve the IPC layout's trailing reserved byte, initialized to zero. */
	osal_memcpy(payload + osal_sizeof(msg_len), idc_infor->local_para_ptr->data, msg_len - 1);
	pSignal = &lxop->signal;
	pSignal->timeoutValue = MAX_EACH_WMT_CMD;
	lxop->op.opId = WMT_OPID_IDC_MSG_HANDLING;
	lxop->op.au4OpData[0] = (SIZE_T)payload;
...
/* The published pointer keeps the consumer reference until it is cleared. */
...
/* Keep the consumer reference through completion publication. */
...
static VOID wmt_lib_wmtd_worker_thread_work_handler(struct work_struct *work)
{
	PUINT8 pbuf = NULL;
	INT32 len = 0;
	UINT32 opId = wmt_lib_active_op_id(&gDevWmt, true);
...
static INT32 wmt_lib_pin_ctrl(WMT_IC_PIN_ID id, WMT_IC_PIN_STATE stat, UINT32 flag)
{
	...
	bRet = wmt_lib_put_act_op_result(pOp, &result);
	ENABLE_PSM_MONITOR();
	if (bRet == MTK_WCN_BOOL_FALSE)
		WMT_WARN_FUNC("PIN_ID(%d) PIN_STATE(%d) flag(%d) fail\n", id, stat, flag);
	else
		WMT_DBG_FUNC("OPID(%d) type(%zu) ok\n", result.opId, result.au4OpData[0]);
...
#define RETRYTIMES 10
	...
	if (src == WMTRSTSRC_RESET_TEST) {
		opId = wmt_lib_active_op_id(pDevWmt, false);
		if (opId == WMT_OPID_FUNC_ON || opId == WMT_OPID_FUNC_OFF) {
	...
	/* wakeup blocked opid */
	wmt_lib_cancel_current_op(pDevWmt);
	/* wakeup blocked cmd */
	wmt_dev_rx_event_cb();
...
/* Borrowed only by the core dispatcher running on the publishing wmtd thread. */
			   ...
			   UINT32 rx_data_buf_len, PUINT32 p_rx_data_len)
{
	...
	if (!p_tx_data_buf || !tx_data_len || tx_data_len > 0xffff ||
	    !p_rx_data_buf || !rx_data_buf_len || rx_data_buf_len > 0xffff ||
	    !p_rx_data_len)
		return -1;
	...
	payload = wmt_lib_alloc_op_data(pOp, tx_data_len + rx_data_buf_len);
	if (!payload) {
		wmt_lib_put_op_to_free_queue(pOp);
		return -1;
	}
	osal_memcpy(payload, p_tx_data_buf, tx_data_len);
	pSignal = &pOp->signal;
	pSignal->timeoutValue = MAX_EACH_WMT_CMD;
	pOp->op.opId = WMT_OPID_GPS_MCU_CTRL;
	pOp->op.au4OpData[0] = (SIZE_T)payload;
	pOp->op.au4OpData[1] = tx_data_len;
	pOp->op.au4OpData[2] = (SIZE_T)(payload + tx_data_len);
	pOp->op.au4OpData[3] = rx_data_buf_len;
	pOp->op.au4OpData[4] = 0;
	if (DISABLE_PSM_MONITOR()) {
		WMT_ERR_FUNC("wake up failed\n");
		wmt_lib_put_op_to_free_queue(pOp);
		return -1;
	...
	bRet = wmt_lib_submit_op_result(pOp, &result);
	ENABLE_PSM_MONITOR();

	if (bRet == MTK_WCN_BOOL_FALSE) {
		WMT_WARN_FUNC("WMT_OPID_GPS_MCU_CTRL fail(%zu)\n", result.au4OpData[5]);
	} else if (result.au4OpData[4] <= rx_data_buf_len) {
		received = result.au4OpData[4];
		osal_memcpy(p_rx_data_buf, payload + tx_data_len, received);
		*p_rx_data_len = received;
		ret = 0;
	}
	wmt_lib_put_op_ref(pOp);
	return ret;
