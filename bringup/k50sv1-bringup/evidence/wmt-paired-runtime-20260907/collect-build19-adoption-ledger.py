...
transformed_path = trial / 'runtime/build19-retained-transform-predecessor/result.json'
transformed = json.loads(transformed_path.read_text())
assert transformed['status'] == 'PASS' and len(transformed['rows']) == 2
transformed_rows = {row['path']: row for row in transformed['rows']}
processed = []
for row in rows:
    ...
    if row['current_payload'] == 'retained_unchanged':
        if digest != row['sha256']:
            old = transformed_rows[row['current_install_path']]
            assert old['original_sha256'] == row['sha256']
            assert old['installed_build17_sha256'] == old['product_sha256'] == digest
            assert old['matches_product19']
...
result = dict(status='PASS', rows=506, counts=counts, build_incremental=expected['incremental'],
              ...
              retained_installed_identical_to_original=474,
              retained_processed_files_identical_to_installed_build17=processed,
              predecessor_readback_sha256=hashlib.sha256(transformed_path.read_bytes()).hexdigest(),
              scope='All original 506 inventory entries are mapped to actual clean product files. '
              '474 retained product files match their recorded original hashes. Two build-transformed '
              'retained files match actual installed build17 bytes; their original hashes remain recorded. '
              'This is product provenance; real-feature claims remain in their runtime evidence.')
result_path.write_text(json.dumps(result, indent=2) + '\n')
print('PASS: 506 files accounted for; 474 original hashes, 2 unchanged predecessor transforms, 30 source replacements')
