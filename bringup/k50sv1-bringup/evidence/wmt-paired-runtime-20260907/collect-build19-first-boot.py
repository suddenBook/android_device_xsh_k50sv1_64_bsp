...
out=trial/'runtime/build19-first-boot-early';out.mkdir(exist_ok=False)
...
old_boot_id=(trial/'runtime/build19-preflash/identity.txt').read_text().splitlines()[1].encode()
steps=[]
streams=[]
...
try:
 ...
 readback=subprocess.run(['python3',str(trial/'collect-installed-readback.py'),'19','first-boot'],capture_output=True)
