...
parser.add_argument('--build', required=True, type=int, choices=[16, 17])
...
assert len(bound) == 1
assert '/sys/module/wmt_drv' in state
...
result = dict(status='PASS', build=args.build, boot_id=identity[1], incremental=identity[0], taint=0,
