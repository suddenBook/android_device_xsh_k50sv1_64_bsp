#!/usr/bin/env python3
"""Build an isolated Java camera probe with the installed Android SDK."""
...
def main():
    ...
    java = BASE / "CameraProbeActivity.java"
    ...
    if not key.exists():
        ...
        try:
            run(["keytool", "-genkeypair", "-alias", "camera-probe", "-keyalg", "RSA",
                 "-keysize", "2048", "-validity", "3650", "-storetype", "PKCS12",
                 "-keystore", key, "-storepass:file", password, "-keypass:file", password,
                 "-dname", "CN=K50 Camera Probe Test Only,O=Local Test,C=XX", "-noprompt"])
        finally:
            os.umask(previous)
    key.chmod(0o600)
    password.chmod(0o600)
    apk = out / "k50-camera-probe.apk"
    ...
    if permissions != ["uses-permission: name='android.permission.CAMERA'"]:
        raise RuntimeError(f"Unexpected camera probe permissions: {permissions}")
    provenance = {
