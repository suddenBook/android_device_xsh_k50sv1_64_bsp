#!/usr/bin/env python3
"""Build an isolated Java sensor probe with the installed Android SDK."""
...
def main():
    ...
    java = BASE / "SensorProbeActivity.java"
    ...
    if not key.exists():
        ...
        try:
            run(["keytool", "-genkeypair", "-alias", "sensor-probe", "-keyalg", "RSA",
                 "-keysize", "2048", "-validity", "3650", "-storetype", "PKCS12",
                 "-keystore", key, "-storepass:file", password, "-keypass:file", password,
                 "-dname", "CN=K50 Sensor Probe Test Only,O=Local Test,C=XX", "-noprompt"])
        finally:
            os.umask(previous)
    key.chmod(0o600)
    password.chmod(0o600)
    apk = out / "k50-sensor-probe.apk"
    ...
    if "uses-permission:" in badging:
        raise RuntimeError("The sensor probe must not request permissions")
    provenance = {
