...
class HalProxy : public ISensors, public IScopedWakelockRefCounter {
  public:
    ...
    // The board wires its single legacy sub-HAL directly into the service.
     ...
     */
    const SensorInfo& getSensorInfo(int32_t sensorHandle) { return mSensors.at(sensorHandle); }
  ...
  private:
    ...
    std::vector<sp<IHalProxyCallback>> mSubHalCallbacks;
